...
After the data have
been converted to the tall table above, we can do a `dcast` to get a
wider data table:
...
* `value.name` is used as the column name for the reshaped data in the output (as in `melt.data.table`).

## Comparison with tidyr

The `nc::capture_melt_single` function was inspired by the `tidyr`
...
In conclusion `nc::capture_melt_single` provides an alternative which
...
if `nc::capture_melt_multiple` is used with `nc::field`.
...
`nc::capture_melt_multiple`. Two differences are that `nc`
